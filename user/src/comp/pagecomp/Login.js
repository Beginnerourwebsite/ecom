    ...
    function login() {
        ...
        Autocreatedatabase(obj, "user", "get").then(datas => {
            let confirmExit = datas.filter(element => {
                return ((element.name == allInputvalue.email || element.email == allInputvalue.email) && element.tp_password == allInputvalue.tp_password)
            })
            ...
            if (confirmExit.length == 0) {
                ...
                console.log(confirmExit)
                sessionStorage.setItem("details", JSON.stringify({ User: confirmExit[0].name, pass: confirmExit[0].email.split("@")[0] }))
                // useEffect(() => {
                let obj = {
                    database: "ecom",
                    table: "product"
                }
                Autocreatedatabase(obj, "column", "get").then(data => {
                    console.log(data)

                    // setTimeout(() => {

                    let details = `${confirmExit[0].name}_${confirmExit[0].email.split("@")[0]}`
                        ...
                        <div className="col-xl-6 col-lg-8">
                            <div className="tp-login-wrapper">
                                <div className="tp-login-top text-center mb-30">
                                    <h3 className="tp-login-title">Login to Us.</h3>
                                    <p>Don’t have an account? <span><Link to="/signUp">Create a free account</Link></span></p>
                                </div>
                                <div className="tp-login-option">
                                    ...
                                    <div className="tp-login-input-wrapper">
                                        <div className="tp-login-input-box">
                                            <div className="tp-login-input">
                                                <input id="email" type="email" onChange={Inputvalue} placeholder="Username/email" />
                                            </div>
                                        ...
                                        <div className="tp-login-input-box">
                                            <div className="tp-login-input">
                                                <input id="tp_password" type="password" onChange={Inputvalue} placeholder="Min. 6 character" />
                                            </div>
                                    ...
                                    <div className="tp-login-bottom">
                                        <button onClick={login} className="tp-login-btn w-100">Login</button>
