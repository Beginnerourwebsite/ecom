...
import Mysql from '../comp/all/Mysql'
import Autocreatedatabase from '../comp/all/Autocreatedatabase'

export default function Signup() {
    let navigate=useNavigate()
    // show hidden password code start
    function showhide(e) {
        let passwordInput = document.getElementById("tp_password")
        let open_eye = document.getElementsByClassName("open-eye")[0]
        let close_eye = document.getElementsByClassName("open-close")[0]
        let selftagId = e.target.id

        if (selftagId == "open-eye") {
            close_eye.style.display = "block"
            open_eye.style.display = "none"
            passwordInput.type = "password"
        }
        else if (selftagId == "close-eye") {
    ...
    // show hidden password code end

    // fetch input box value code start
    let [allInputvalue, setallInputvalue] = useState({})
    function Inputvalue(e) {
        let selfTag = e.target
        setallInputvalue({ ...allInputvalue, [selfTag.id]: selfTag.value })
    }
    // fetch input box value code end



    // sign up code start
    function registernewaccoutn() {

        let agreeornot = document.getElementById("remeber").checked

        if (agreeornot == true) {

            let obj = {
                list: Object.keys(allInputvalue),
                database: "login",
                table: "account",
                ifnot:"create"
            }
            // let allkeys = 
            // let databse
            // Autocreatedatabase(obj,"user", "post", allInputvalue).then(getstatus => {
            //     console.log(getstatus)
            // })
            Autocreatedatabase(obj, "user", "get").then(getstatus => {
                // console.log(getstatus)
                let Exitalready = getstatus.filter(element => {
                    // console.log(element)
                    return (element.name == allInputvalue.name && element.tp_password == allInputvalue.tp_password)
                })
                // console.log(dat)
                if (Exitalready.length == 0) {
                    Autocreatedatabase(obj, "user", "post", {...allInputvalue,"email":(allInputvalue.email)}).then(getstatus => {
                        navigate("/")        
                    })
                }
                else alert("already exit")
            }).catch(eroo => {
                Autocreatedatabase(obj, "user", "post", allInputvalue).then(getstatus => {
                    console.log(getstatus)
                })
        ...
        <div><main>
            ...
            <section className="tp-login-area pb-140 p-relative z-index-1 fix">
                <div className="tp-login-shape">
                    <img className="tp-login-shape-1" src="assets/img/login/login-shape-1.png" />
                    <img className="tp-login-shape-2" src="assets/img/login/login-shape-2.png" />
                    <img className="tp-login-shape-3" src="assets/img/login/login-shape-3.png" />
                    <img className="tp-login-shape-4" src="assets/img/login/login-shape-4.png" />
                </div>
                <div className="container">
                        ...
                        <div className="col-xl-6 col-lg-8">
                            <div className="tp-login-wrapper">
                                <div className="tp-login-top text-center mb-30">
                                    <h3 className="tp-login-title">Sign Up Us.</h3>
                                ...
                                <div className="tp-login-option">
                                    ...
                                    <div className="tp-login-input-wrapper">
                                        <div className="tp-login-input-box">
                                            <div className="tp-login-input">
                                                <input id="name" onChange={Inputvalue} type="text" placeholder="Shahnewaz Sakil" />
                                            </div>
                                        ...
                                        <div className="tp-login-input-box">
                                            <div className="tp-login-input">
                                                <input id="email" onChange={Inputvalue} type="email" placeholder="shofy@mail.com" />
                                            </div>
                                            <div className="tp-login-input-title">
                                        ...
                                        <div className="tp-login-input-box">
                                            <div className="tp-login-input">
                                                <input id="tp_password" onChange={Inputvalue} type="password" placeholder="Min. 6 character" />
                                            </div>
                                            <div className="tp-login-input-eye" id="password-show-toggle">
                                                <span onClick={showhide} id="open-eye" className="open-eye">
                                                    <svg onClick={showhide} id="open-eye" width={18} height={14} viewBox="0 0 18 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                <span id="close-eye" onClick={showhide} className="open-close">
                                                    <svg width={19} id="close-eye" onClick={showhide} height={18} viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div className="tp-login-bottom">
                                        <button onClick={registernewaccoutn} className="tp-login-btn w-100">Sign Up</button>
                                    </div>
                                </div>
                            </div>
                        </div>
