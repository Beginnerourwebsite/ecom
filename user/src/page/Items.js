...
import Product from '../comp/pagecomp/Product'
import { Link } from 'react-router-dom'
import Test from '../Test'
import Pagenation from '../comp/pagecomp/Pagenation'
import Context from '../comp/all/Context'
import Autocreatedatabase from '../comp/all/Autocreatedatabase'

export default function Items() {

    let [Viewbyid, setViewbyid] = useState("0")
    let [items, setdatas] = useState(0)

    let [insertdata, setinsertdata] = useState([{ Image: [{ url: "" }] }
    ...
    useEffect(() => {
        ...
        Autocreatedatabase(obj, "user", "get").then(CartItem => {
            ...
            fetch("http://localhost:5000/").then(data => data.json().then(Allitem => {
              
                let arr = []
                Allitem.map((element, index) => {

                    CartItem.find(data => {
                        if (data.id == element.id) {
                            Allitem[index].carts= "active"
                            Allitem[index].items= 1
                            console.log(data)
                        }
                        // console.log(details)
                    ...
                    // element.Image = JSON.parse(element.Image.toString())
                    // arr.push(element)
                    // Allitem[index].Image = 
                ...
                setinsertdata(Allitem)
                setViewbyid(Allitem.length)
            })
    ...
    //             show.api.forEach((element, index) => {
    //                 if (show.api[index].id) {
    //                     // delete obj.list.id
    //                     let senddata = ({ ...element, Image: JSON.stringify(element.Image[0]), Offer: JSON.stringify(element.Offer) })
    //                     Autocreatedatabase(obj, "user", "post", senddata).then(data => {
                        ...
                        <div className="row">
                            ...
                            <div className="col-xl-12 col-lg-12">
                                <div className="tp-shop-main-wrapper">
                                    ...
                                    <Product insertdata={insertdata} totalitemset={setdatas} />
